...
Функция СозданиеСпискаНаСервере(Дата, СоздатьОбъект) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Если СоздатьОбъект Тогда
			Реализация = СозданиеПерезаполнениеРеализации(Выборка, КонецМесяца(Дата), ЗначениеЗаполнено(Выборка.Реализация));
...
Функция СозданиеПерезаполнениеРеализации(Выборка, Дата, ЕстьРеализация)
