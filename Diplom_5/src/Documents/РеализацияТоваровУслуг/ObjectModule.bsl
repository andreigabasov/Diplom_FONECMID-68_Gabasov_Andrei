...
Процедура ВКМ_ВыполнитьАвтозаполнение() Экспорт  
	...
	Если Не ЗначениеЗаполнено(АбонентскаяПлата) Тогда
		ОбщегоНазначения.СообщитьПользователю(Нстр("ru = 'Абонентская плата не заполнена'"));
		Возврат;
	КонецЕсли;
	Если Не ЗначениеЗаполнено(РаботыСпециалиста) Тогда
		ОбщегоНазначения.СообщитьПользователю(Нстр("ru = 'Работы специалиста не заполнены'"));
		Возврат;
	КонецЕсли;
	...
	|	ВКМ_ВыполненныеКлиентуРаботыОбороты.ВКМ_Договор.ВКМ_СтавкаЧаса КАК СтавкаЧаса
	|ИЗ
	...
	Пока Выборка.Следующий() Цикл
		Если Выборка.Сумма > 0 Тогда
			...
			НоваяСтрока.Цена = Выборка.СтавкаЧаса;
